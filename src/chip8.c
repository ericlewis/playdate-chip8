...
#define debug_print(fmt, ...)                         \
    do {                                              \
        if (DEBUG) pd->system->logToConsole(fmt, __VA_ARGS__); \
    } while (0)
...
unsigned char display_2[64 * 32] = {0};
 ...
 * @return 0 if success, -1 if read failure, errno if failure
 * */
int load_rom(char* filename) {
    ...
    FileStat st;
    pd->file->stat(filename, &st);
    size_t fsize = st.size;

    size_t bytes_read = pd->file->read(fp, memory + 0x200, fsize);
...
void emulate_cycle(void) {
    ...
    switch (op & 0xF000) {
        ...
        case 0x3000:
            ...
            // (big-endian) a right shift by 8 increases the byte address by 1
            if (V[x] == (op & 0x00FF)) {
                pc += 2;
            }
        ...
        case 0x8000:
            switch (op & 0x000F) {
                ...
                default:
                    pd->system->logToConsole("[FAILED] Unknown op: 0x%X", op);
                    break;
            }
            break;
        ...
        case 0xD000:
            ...
            for (int yline = 0; yline < height; yline++) {
                ...
                for (int xline = 0; xline < 8; xline++) {
                    ...
                    // xline scans through the byte, one bit at the time)
                    if ((px & (0x80 >> xline)) != 0) {
        ...
        default:
            debug_print("[FAILED] Unknown opcode: 0x%X", op);
            break;
    }
